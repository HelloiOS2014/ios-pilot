...
```bash
cd ios-pilot
make deploy    # 编译 + 安装到 ~/.local/bin/ios-pilot
```
...
`make deploy` 会自动将 Skill 安装到 `~/.claude/skills/ios-pilot/`，在任何项目中即可使用 ios-pilot 工作流。

也可以手动安装：
```bash
mkdir -p ~/.claude/skills/ios-pilot
cp skills/ios-pilot/SKILL.md ~/.claude/skills/ios-pilot/SKILL.md
```
